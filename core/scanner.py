...
class MarketScanner:
    ...
    def _fetch_by_slug_pattern(self):
        ...
        timestamps = [current_window - 600, current_window - 300, current_window, current_window + 300]
    ...
    def _search_gamma_api(self):
        markets = []
        try:
            params = {"active": "true", "closed": "false", "limit": 20, "order": "endDate", "ascending": "true"}
    ...
    def get_market_trades(self, condition_id):
        try:
            resp = self.session.get(f"{CLOB_API}/trades", params={"condition_id": condition_id}, timeout=10)
            resp.raise_for_status()
            return resp.json()
        except Exception as e:
